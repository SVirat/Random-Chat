...
module.exports = (io, app) => {
  io.on('connection', (socket) => {
    let windowID = socket;
    socket.name = findName();
    var userName = (socket.name).replace("\r", "");
    var quote = findQuote();
    socket.emit('wait', { "quote": quote, "user": userName, "message": "Please wait... connecting you a stranger."});
          ...
          // Make a user object and add it to the onlineUsers list and rooms too(maybe we can add to room once we have the partner.)
            ...
            socket.on('privateRoom', (user) => {
              ...
              try {
                ...
                io.sockets.in(socket.roomID).emit('toast', { "user": userName, "message": "Connection successful!"});
              }
              catch(e) {
    ...
    socket.on('sendMessage', (data) => {
      ...
      io.sockets.in(data.room).emit('newMessage', { "user": userName, "message": data , "senderId": windowID.id, "timeStamp": timeStamp});
    });

    socket.on('typing', (data) => {
        io.sockets.in(data.room).emit('addTyping', { "user": userName, "senderId": windowID.id, "typingStatus": data.typingStatus });
    });
    ...
    socket.on('disconnect', () => {
      ...
      if(index >= 0){
        if(rooms[index].isFilled == true){
          let warning = { "title": "Stranger has disconnected.", "message": "Please click on 'New' button to connect to someone else." };
          io.sockets.in(windowID.roomID).emit('alone', { "user": userName, "warning": warning, "roomID": windowID.roomID });
  ...
  function findQuote() {
    var fs = require('fs');
    var readline = require('readline');
    var quotes = fs.readFileSync('quotes.txt').toString('utf-8').split('\n');
